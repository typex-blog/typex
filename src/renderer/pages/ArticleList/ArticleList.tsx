...
import { AntCloudOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
...
const ArticleList: React.FC<ArticleListProps> = props => {
  ...
  const onArticleEdit: Function = (d: Article) => {
    return () => {
      history.push(`/article/edit/${d._id}`);
        ...
        return (
          <div>
            ...
            <Popconfirm title="您确认删除该文章吗？" onConfirm={onArticleDelete(d)}>
              <Tooltip title="删除">
                <Button type="primary" danger shape="circle" icon={<DeleteOutlined />} className="delBtn" />
              </Tooltip>
            </Popconfirm>
          </div>
        );
  ...
  const taskColumns: ColumnProps<any>[] = [
    {
      ...
      render: (text, d) => {
        if (d.name === constants.platform.JUEJIN) {
          return <img className="siteLogo" alt={d.label} src={imgJuejin} />;
        } else if (d.name === constants.platform.SEGMENTFAULT) {
          return <img className="siteLogo" alt={d.label} src={imgSegmentfault} />;
        } else if (d.name === constants.platform.JIANSHU) {
          return <img className="siteLogo" alt={d.label} src={imgJianshu} />;
        } else if (d.name === constants.platform.CSDN) {
          return <img className="siteLogo" alt={d.label} src={imgCsdn} />;
        } else if (d.name === constants.platform.ZHIHU) {
          return <img className="siteLogo" alt={d.label} src={imgZhihu} />;
        } else if (d.name === constants.platform.OSCHINA) {
          return <img className="siteLogo" alt={d.label} src={imgOschina} />;
        } else if (d.name === constants.platform.TOUTIAO) {
          return <img className="siteLogo" alt={d.label} src={imgToutiao} />;
        } else if (d.name === constants.platform.CNBLOGS) {
          return <img className="siteLogo" alt={d.label} src={imgCnblogs} />;
        } else if (d.name === constants.platform.V2EX) {
          return <img className="siteLogo" alt={d.label} src={imgV2ex} />;
        } else if (d.name === constants.platform.WECHAT) {
          return <img className="siteLogo" alt={d.label} src={imgWechat} />;
        } else {
          return <div />;
        }
      },
  ...
  return (
    <div>
      ...
      <div style={ { textAlign: 'right', marginBottom: '10px', marginTop: '10px', marginRight: '10px' } }>
        <Button className="addBtn" type="primary" onClick={onArticleCreate}>
          创建文章
        </Button>
      </div>
      <Card>
