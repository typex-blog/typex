  ...
  const columns: ColumnProps<any>[] = [
    {
      ...
      render: (text: string, d: Platform) => {
        ...
        if (d.name === constants.platform.JUEJIN) {
          img = <img className="siteLogo" src={ imgJuejin }/>;
        } else if (d.name === constants.platform.SEGMENTFAULT) {
          img = <img className="siteLogo" src={ imgSegmentfault }/>;
        } else if (d.name === constants.platform.JIANSHU) {
          img = <img className="siteLogo" src={ imgJianshu }/>;
        } else if (d.name === constants.platform.CSDN) {
          img = <img className="siteLogo" src={ imgCsdn }/>;
        } else if (d.name === constants.platform.ZHIHU) {
          img = <img className="siteLogo" src={ imgZhihu }/>;
        } else if (d.name === constants.platform.OSCHINA) {
          img = <img className="siteLogo" src={ imgOschina }/>;
        } else if (d.name === constants.platform.TOUTIAO) {
          img = <img className="siteLogo" src={ imgToutiao }/>;
        } else if (d.name === constants.platform.CNBLOGS) {
          img = <img className="siteLogo" alt={ d.label } src={ imgCnblogs }/>;
        } else if (d.name === constants.platform.V2EX) {
          img = <img className="siteLogo" alt={ d.label } src={ imgV2ex }/>;
      ...
      >
        <Form labelCol={ { sm: { span: 4 } } } wrapperCol={ { sm: { span: 20 } } }>
          ...
          <Form.Item label="编辑器类别">
            ...
            >
              <Select.Option key={ constants.editorType.MARKDOWN } value="">Markdown</Select.Option>
              <Select.Option key={ constants.editorType.RICH_TEXT } value="">富文本编辑</Select.Option>
            </Select>
          </Form.Item>
          <Form.Item label="描述">
            <Input.TextArea
